...
export default function MaintenanceScreen({ settings }: MaintenanceScreenProps) {
  ...
  let maintenanceMsg = {
    ar: 'الموقع حالياً قيد الصيانة لإجراء بعض التحديثات والتحسينات المجدولة. سنعود للعمل قريباً. نشكر تفهمكم.',
  ...
  if (settings?.maintenanceMessage) {
    try {
      const parsed = JSON.parse(settings.maintenanceMessage);
      if (parsed && (parsed.ar || parsed.en)) {
        maintenanceMsg = { ...maintenanceMsg, ...parsed };
      } else if (typeof settings.maintenanceMessage === 'string') {
        maintenanceMsg = {
          ar: settings.maintenanceMessage,
          en: settings.maintenanceMessage
        };
      }
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white flex flex-col justify-between p-6 relative overflow-hidden font-sans" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <main className="w-full max-w-2xl mx-auto flex-1 flex flex-col items-center justify-center text-center z-10 py-12">
        ...
        >
          {isRtl ? 'المنصة قيد التحديث والصيانة المجدولة' : 'Platform Under Scheduled Maintenance'}
        </motion.h1>
        ...
        >
          ...
          >
            ...
            <span>{isRtl ? 'تسجيل دخول الإدارة والمسؤولين' : 'Staff & Admin Portal'}</span>
